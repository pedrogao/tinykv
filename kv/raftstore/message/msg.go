...
const (
	// MsgTypeNull just a placeholder
	MsgTypeNull MsgType = 0
	// MsgTypeStart message to start the ticker of peer
	MsgTypeStart MsgType = 1
	// MsgTypeTick message of base tick to drive the ticker
	MsgTypeTick MsgType = 2
	// MsgTypeRaftMessage message wraps a raft message that should be forwarded to Raft module
	// the raft message is from peer on other store
	MsgTypeRaftMessage MsgType = 3
	// MsgTypeRaftCmd message wraps a raft command that maybe a read/write request or admin request
	// the raft command should be proposed to Raft module
	MsgTypeRaftCmd MsgType = 4
	// MsgTypeSplitRegion message to trigger split region
	...
	// MsgTypeRegionApproximateSize message to update region approximate size
	...
	// MsgTypeStoreTick message of store base tick to drive the store ticker, including store heartbeat
	MsgTypeStoreTick MsgType = 106
	// MsgTypeStoreStart message to start the ticker of store
